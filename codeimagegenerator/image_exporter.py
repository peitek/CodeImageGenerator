...
def create_image_from_code(language, function_name, code, code_task=""):
    ...
    if code_task:
        ...
        draw.text((IMAGE_SIZE_X / 2 - instruction_width / 2, (IMAGE_SIZE_Y / 30)), code_task, FONT_COLOR_INSTRUCTION,
                  font=inconsolata_instructions)
    else:
    ...
    (_, height_of_font) = ImageDraw.ImageDraw(image).textsize(text='placeholder text', font=inconsolata)
...
def write_image_to_file(file_name, image):
    ...
    if not os.path.isdir(OUTPUT_DIRECTORY):
        try:
            ...
            print("Error creating the output repository", e)
